...
function BoundingBox({ label, children, className, score = "sys.ok" }) {
  return (
    <div className={`relative border border-violet-500/40 bg-black/50 backdrop-blur-sm ${className}`}>
      <div className="absolute -top-[1px] -left-[1px] bg-violet-500 text-black text-[10px] font-mono font-bold px-2 py-0.5 flex items-center gap-2">
        <span>{label}</span>
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4 relative overflow-hidden cursor-crosshair">
      ...
      <div className="w-full max-w-md relative z-10">
        <BoundingBox label="AUTH_MODULE" score="1.00" className="p-8 pb-10">
          <div className="mb-10 border-b border-white/10 pb-6">
            <h1 className="text-2xl font-bold uppercase tracking-tight flex items-center gap-2">
              <Zap className="w-5 h-5 text-violet-500" />
              AUTHENTICATE
            </h1>
            <p className="text-gray-500 font-mono text-xs mt-2 uppercase">Provide credentials to access workspace.</p>
          </div>
