...
function StatCard({ icon: Icon, label, value, color }) {
    return (
        <div className={cn(
            "relative flex flex-col items-center justify-center p-5 rounded-none border border-white/[0.07]",
            "bg-white/[0.02] backdrop-blur-sm hover:bg-white/[0.04] transition-all duration-300 group"
        )}>
            <div className={cn("absolute inset-0 rounded-none opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl -z-10", color)} />
            <div className={cn("w-10 h-10 rounded-none flex items-center justify-center mb-3 border border-white/10", color)}>
    ...
    return (
        <div className="h-full space-y-6 max-w-6xl mx-auto animate-fade-in pb-10">
            <div className="flex items-center justify-between mb-4">
                ...
                {onNavigate && (
                    <button 
                        ...
                        className="px-4 py-2 bg-violet-500 hover:bg-indigo-700 text-white rounded-none font-medium transition-colors"
                    >
                        Show the images
                    </button>
                )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard icon={ImageIcon} label="Total Images" value={stats?.total_images || 0} color="bg-violet-400/10 text-violet-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="rounded-none border border-white/[0.07] bg-white/[0.02] backdrop-blur-sm overflow-hidden flex flex-col">
                                    ...
                                    return (
                                        <div key={cls} className="space-y-1.5">
                                            ...
                                            <div className="h-2 bg-white/5 rounded-none overflow-hidden">
                                                <div 
                                                    className="h-full bg-violet-500 rounded-none transition-all duration-1000" 
                                                    style={{ width: `${pct}%` }} 
                                                />
                                            </div>
                                        </div>
                            ...
                            <div className="space-y-6">
                                <div className="flex items-start justify-between">
                                    ...
                                    <Badge className={cn("capitalize border text-xs", 
                                        ...
                                        latestJob.status === "running" ? "bg-violet-400/10 text-violet-400 border-violet-500/30" :
                                ...
                                {latestJob.status === "completed" && latestJob.metrics?.metrics ? (
                                    <div className="grid grid-cols-3 gap-3">
                                        <div className="bg-white/5 rounded-none p-3 text-center border border-white/10">
                                    ...
                                    <div className="flex flex-col items-center justify-center py-6">
                                        <div className="w-full bg-white/5 rounded-none h-2 mb-2 overflow-hidden">
                                            <div className="bg-violet-400 h-2 rounded-none transition-all" style={{ width: `${latestJob.progress || 0}%` }} />
                                        </div>
