...
function StatPill({ label, value, color = "indigo" }) {
  ...
  return (
    <div className={`flex flex-col items-center px-4 py-3 rounded-xl border ${colorMap[color]}`}>
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center text-center gap-6 p-12">
        <div className="w-20 h-20 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center">
          <FlaskConical className="w-9 h-9 text-indigo-400" />
        </div>
        <div>
        ...
        <div className="flex flex-col gap-4 overflow-y-auto pr-1" style={{ scrollbarWidth: "none" }}>
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={fetchModels} className="text-gray-500 hover:text-gray-300 transition-colors p-1 rounded-lg hover:bg-white/5">
            ...
            <div className="relative" data-model-selector>
              <button
                ...
                className="w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl border border-white/10 bg-white/5 hover:bg-white/8 text-sm text-left transition-all"
              >
              ...
              {modelOpen && (
                <div className="absolute z-50 top-full mt-1.5 w-full bg-zinc-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden">
                  {models.map(m => (
                    <button
                      key={m.name}
                      onClick={() => { setSelectedModel(m.name); setModelOpen(false); }}
                      className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm text-left transition-colors hover:bg-white/5 ${selectedModel === m.name ? "text-indigo-300 bg-indigo-500/10" : "text-gray-300"}`}
                    >
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 ${selectedModel === m.name ? "bg-indigo-400" : "bg-gray-600"}`} />
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative h-1.5 rounded-full bg-white/10">
                <div
                  className="absolute left-0 top-0 h-full rounded-full bg-gradient-to-r from-violet-500 to-violet-400 transition-all"
          ...
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4 space-y-3">
            ...
            <button
              ...
              disabled={uploading}
              className="w-full flex items-center justify-center gap-2 h-10 rounded-xl border border-dashed border-indigo-500/40 bg-indigo-500/5 hover:bg-indigo-500/10 hover:border-indigo-500/60 text-indigo-300 text-sm font-medium transition-all disabled:opacity-50"
            >
              {uploading
          ...
          {testImages.length > 0 && (
            <Button
              ...
              className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 border-0 h-10 text-sm font-semibold"
            >
              {loading
        ...
        <div className="min-h-0 flex flex-col gap-4 overflow-hidden">
          ...
          {selectedResult ? (
            <div className="flex flex-col gap-3 h-full">
              ...
              <div className="flex gap-4 flex-1 min-h-0">
                {/* canvas */}
                <div className="flex-1 rounded-2xl overflow-hidden bg-black/40 border border-white/8 flex items-center justify-center min-h-0">
                  <canvas
                    ref={canvasRef}
                ...
                {selectedResult.result?.predictions?.length > 0 && (
                  <div className="w-52 flex-shrink-0 rounded-2xl border border-white/8 bg-white/[0.03] p-3 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
                    ...
                    <div className="space-y-1.5">
                      {selectedResult.result.predictions.map((pred, idx) => (
                        <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-white/5 border border-white/5">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ backgroundColor: classColor(idx) }} />
          ...
          /* empty state: no images */
            ...
            <div
              ...
              onDrop={onDrop}
              onClick={() => fileInputRef.current?.click()}
              className={`flex-1 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center text-center cursor-pointer transition-all gap-4 p-12
                ${dragging
                  ? "border-indigo-500/70 bg-indigo-500/5 scale-[1.01]"
                  : "border-white/10 hover:border-indigo-500/40 hover:bg-white/[0.02]"
                }`}
            >
              <div className={`w-20 h-20 rounded-2xl flex items-center justify-center transition-all ${dragging ? "bg-indigo-500/20" : "bg-white/5 border border-white/10"}`}>
                <Upload className={`w-9 h-9 transition-colors ${dragging ? "text-indigo-400" : "text-gray-500"}`} />
              </div>
              <div>
              ...
              {!dragging && (
                <span className="text-xs text-gray-600 border border-white/5 rounded-full px-4 py-1.5">
          ...
          /* image grid */
            ...
            <div
              ...
              onDragLeave={onDragLeave}
              onDrop={onDrop}
              className={`flex-1 overflow-y-auto rounded-2xl transition-all ${dragging ? "ring-2 ring-indigo-500/50 ring-offset-2 ring-offset-black" : ""}`}
            ...
            >
              <div className="grid grid-cols-2 xl:grid-cols-3 gap-3 pb-4">
                ...
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="aspect-[4/3] rounded-2xl border-2 border-dashed border-white/10 hover:border-indigo-500/40 hover:bg-indigo-500/5 flex flex-col items-center justify-center gap-2 text-gray-500 hover:text-indigo-400 transition-all"
                >
...
function ImageCard({ img, selectedModel, onRun, onRemove, onView }) {
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden border border-white/8 bg-white/[0.03] hover:border-white/15 transition-all">
      {/* image */}
      <div className="aspect-[4/3] relative overflow-hidden bg-black/40">
        ...
        <button
          ...
          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 w-6 h-6 rounded-full bg-red-500/90 hover:bg-red-400 flex items-center justify-center transition-all"
        >
        ...
        {hasResult && (
          ...
          >
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/15 backdrop-blur-sm text-white text-xs font-medium border border-white/20">
      ...
      <div className="px-3 py-2.5 flex items-center justify-between gap-2">
        ...
        {hasResult ? (
          ...
          <button
            ...
            className="flex-shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white text-[10px] font-semibold transition-all"
          >
            {img.loading
